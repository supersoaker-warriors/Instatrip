...
function picsCtrl ($scope, Getdata, $rootScope, $window){
  ...
  // function getPhotoAt(num) {
  //   Getdata.markMap(num);
  // }

  for (var i = 0; i < $scope.imgs.length; i++) {
    var photomoved = 'photo.moved_' + i;
    $scope.$on(photomoved, function() {
      Getdata.markMap(i);
    });
  }
    ...
    //                 return Getdata.currentImages
    //               },
    //               function() {
    //                 console.log('watching');
    //                 $scope.changeImage();
    //               }, true);
    $scope.$on('updatedPhotos', function(event, data) {
      $scope.imgs = data;
    });
}
